...
void TraceErreur1(const char* pFichier,int Ligne,const char *pTrace, ... ) {
	char Buffer[255];
	va_list arg;
	va_start(arg,pTrace);
	vsprintf(Buffer,pTrace,arg);
	...
	cerr << "(" << pFichier << " - " << Ligne << ") " << Buffer << endl;
	#else
	fprintf(stderr,"(%s - %d)",pFichier,Ligne);
	perror(Buffer);
	va_end(arg);
	#endif
	return;
...
void TraceConstructeur1(const char* NomClass,const char *pTrace,... ) {
	char Buffer[255];
	va_list arg;
	va_start(arg,pTrace);
	vsprintf(Buffer,pTrace,arg);
	...
	if (LgClass < 10) {
		...
		Inter[LgClass + 2] = '\0';
		sprintf(B,"%s",&Inter[2]);
		strcpy(Inter,&NomClass[LgClass + 3]);
	}
	LgClass = atoi(Inter); 
	if (!LgClass) {
		cout << "\033[32;01m[" << B << "] \033[0m"<<  Buffer  << endl;
		return;
	}
...
void TraceMethode1(const char* NomClass,const char* Fonction,const char *pTrace,... ) {
	...
	LgClass = atoi(Inter); 
	if (!LgClass) {
		cout << "\033[34;01m[" << B << ":" << Fonction << "] \033[0m"<<  Buffer  << endl;
		return;
	}
	strcat(B,"<");
	...
	strcat(B,">");
	cout << "\033[34;01m[" << B << ":" << Fonction << "] \033[0m"<<  pTrace  << endl;
