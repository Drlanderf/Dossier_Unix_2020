...
void TraceErreur1(const char*,int,const char*,...);

#define Trace(...) Trace1(__FILE__,__LINE__,__VA_ARGS__)
#define TraceErreur(...) TraceErreur1(__FILE__,__LINE__,__VA_ARGS__)
void Titre(const char *,...);
#ifdef CPP
#define AAA (const char*)typeid(*this).name()
#define TraceConstructeur(...) TraceConstructeur1(AAA,__VA_ARGS__)
#define TraceMethode(...) TraceMethode1(AAA,__FUNCTION__,__VA_ARGS__)
void TraceConstructeur1(const char*,const char*,...);
